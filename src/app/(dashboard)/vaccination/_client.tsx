...
export default function VaccinationClient({ initialData }: Props) {
  const { toast } = useToast();
  const [records, setRecords] = useState<VaccinationSchedule[]>(initialData);
  ...
  // Only re-fetch when showDone changes, not on initial mount
  const handleToggleDone = () => {
    const next = !showDone;
    setShowDone(next);
    setFetching(true);
    fetch(`/api/v1/vaccination?includeDone=${next}`)
      .then((r) => r.json())
      .then((json) => setRecords(json.data ?? []))
      .finally(() => setFetching(false));
  };

  void fetchData; // keep for mutation re-fetches below
  void skipFirst;
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      const refresh = await fetch(`/api/v1/vaccination?includeDone=${showDone}`);
  ...
  return (
    <>
      ...
      <div className="px-4 py-4 space-y-3">
        ...
        {fetching ? <Skeleton className="h-20 w-full rounded-xl" /> :
          records.length === 0 ? (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          {r.isDone ? <CheckCircle className="w-4 h-4 text-green-500" /> : <Clock className="w-4 h-4 text-amber-500" />}
                      ...
                      <div className="flex gap-1 ml-2">
                        {!r.isDone && (
                          <button onClick={() => markDone(r.id, true)} className="p-2 rounded-lg hover:bg-green-50 text-green-500">
