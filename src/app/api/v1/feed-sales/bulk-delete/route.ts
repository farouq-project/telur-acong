...
import { deleteFeedSales } from "@/services/feed.service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const count = await deleteFeedSales(ids.map((id) => String(id)));

    return NextResponse.json({ success: true, count });
  } catch (error) {
    console.error("[feed-sales/bulk-delete POST]", error);
