...
import { deleteFeedPurchases } from "@/services/feed.service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const count = await deleteFeedPurchases(ids.map((id) => String(id)));

    return NextResponse.json({ success: true, count });
  } catch (error) {
    console.error("[feed-purchases/bulk-delete POST]", error);
